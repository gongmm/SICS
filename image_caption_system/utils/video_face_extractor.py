...
class FaceExtractor:
    ...
    def process_videos(self, input_dir, filenames, video_idxs, use_small_tiles, batch_size=30):
        ...
        for video_idx in video_idxs:
            ...
            if use_small_tiles:
                my_tiles, my_resize_info = self._tile_frames_bak(my_frames, target_size)
            else:
        ...
        for i in range(0, len(batch) // batch_size):
            ...
            sub_detections = self.facedet.predict_on_batch(sub_batch, apply_nms=False)
        ...
        if start_idx < len(batch):
            sub_batch = batch[start_idx:len(batch)]
            sub_detections = self.facedet.predict_on_batch(sub_batch, apply_nms=False)
            all_detections.extend(sub_detections)
        ...
        for v in range(len(tiles)):
            ...
            if use_small_tiles:
                detections = self._resize_detections_bak(detections, target_size, resize_info[v])
            else:
            ...
            if use_small_tiles:
                detections = self._untile_detections_bak(num_frames, frame_size, detections)
            else:
            ...
            for i in range(len(detections)):
                ...
                faces, boxes = self._crop_faces(frames[v][i], faces)
    ...
    def process_video(self, video_path, use_small_tiles):
        """Convenience method for doing face extraction on a single video."""
        input_dir = os.path.dirname(video_path)
        filenames = [os.path.basename(video_path)]
        return self.process_videos(input_dir, filenames, [0], use_small_tiles)
    ...
    def _tile_frames_bak(self, frames, target_size):
        num_frames, H, W, _ = frames.shape

        split_size = [min(H, W) // 2, min(H, W) // 2]
        overlap_x = 0.8
        overlap_y = 0.8
        x_step = round(split_size[1] * (1 - overlap_x))
        y_step = round(split_size[0] * (1 - overlap_y))
        num_h = (W - split_size[1]) // x_step
        num_v = (H - split_size[0]) // y_step
        ...
        for f in range(num_frames):
            ...
            for v in range(num_v):
                x = 0
                for h in range(num_h):
                    crop = frames[f, y:y + split_size[0], x:x + split_size[1], :]
                    splits[i] = cv2.resize(crop, target_size, interpolation=cv2.INTER_AREA)
                    resize_info.append(
                        [split_size[1] / target_size[0], split_size[0] / target_size[1], h * x_step, v * y_step])
                    x += x_step
                    i += 1
                y += y_step
    ...
    def _resize_detections_bak(self, detections, target_size, resize_info):
        projected = []
        target_w, target_h = target_size

        for i in range(len(detections)):
            detection = detections[i].clone()
            scale_w, scale_h, offset_x, offset_y = resize_info[i]

            # ymin, xmin, ymax, xmax
            for k in range(2):
                detection[:, k * 2] = (detection[:, k * 2] * target_h) * scale_h + offset_y
                detection[:, k * 2 + 1] = (detection[:, k * 2 + 1] * target_w) * scale_w + offset_x

            # keypoints are x,y
            for k in range(2, 8):
                detection[:, k * 2] = (detection[:, k * 2] * target_w) * scale_w + offset_x
                detection[:, k * 2 + 1] = (detection[:, k * 2 + 1] * target_h) * scale_h + offset_y
    ...
    def _untile_detections_bak(self, num_frames, frame_size, detections):
        """With N tiles per frame, there also are N times as many detections.
        This function groups together the detections for a given frame; it is
        the complement to tile_frames().
        """
    ...
    def _crop_faces(self, frame, detections):
        ...
        for i in range(len(detections)):
            ...
            ymax = min(ymin + size, h)
            xmax = min(xmin + size, w)
            face = frame[ymin:ymax, xmin:xmax, :]
            bounding_box_dict = {"y_min": ymin,
                                 "y_max": ymax,
                                 "x_min": xmin,
                                 "x_max": xmax,
                                 }
            box_list.append(bounding_box_dict)
            faces.append(face)
        return faces, box_list
    ...
    def keep_only_better_face(self, crops, threshold=0.9):
        """For each frame, only keeps the face with the highest confidence.
        ...
        for i in range(len(crops)):
            frame_data = crops[i]
            if len(frame_data["faces"]) > 0:
                face_list = frame_data["faces"][:1]
                score_list = frame_data["scores"][:1]
                box_list = frame_data["boxes"][:1]
                for idx, face in enumerate(frame_data["faces"]):
                    if idx == 0:
                        continue
                    score = frame_data["scores"][idx]
                    box = frame_data["boxes"][idx]
                    if score >= threshold:
                        face_list.append(face)
                        box_list.append(box)
                        score_list.append(score)
                frame_data["faces"] = face_list
                frame_data["scores"] = score_list
                frame_data["boxes"] = box_list
...
class BlazeFacesExtractorVideo:
    """class for extracting faces from a video file via blaze."""

    def __init__(self, blaze_weights, blaze_anchors, input_size=244, frames_per_video=-1):
        # get device
        ...
        facedet.load_weights(blaze_weights)
        facedet.load_anchors(blaze_anchors)
        _ = facedet.train(False)

        video_reader = VideoReader()
        video_read_fn = lambda x: video_reader.read_frames(x, num_frames=frames_per_video)
        self.face_extractor = FaceExtractor(video_read_fn, facedet)
        self.input_size = input_size
        self.frames_per_video = frames_per_video

    def video_face_extractor_bak(self, filename, use_small_tiles=False):

        faces = self.face_extractor.process_video(filename, use_small_tiles)
            ...
            for frame_data in faces:
                for face in frame_data["faces"]:
                    ...
                    if n < self.frames_per_video:
                        ...
                        print("WARNING: have %d faces but batch size is %d" % (n, self.frames_per_video))
            print("face num of %s:%d" % (filename, num))
            return x, n
        else:
            return [], 0

    def video_face_extractor(self, filename, use_small_tiles=False, threshold=0.9):
        # video info
        info_list = self.face_extractor.process_video(filename, use_small_tiles)
        self.face_extractor.keep_only_better_face(info_list, threshold=threshold)
        face_list = []
        box_list = []
        frame_list = []
        if len(info_list) > 0:
            for frame_data in info_list:
                if len(frame_data["faces"]) == 0:
                ...
                for idx, face in enumerate(frame_data["faces"]):
                    ...
                    face = cv2.cvtColor(face, cv2.COLOR_BGR2RGB)
                    face = cv2.resize(face, (self.input_size, self.input_size))
                    single_frame_face.append(face)
                face_list.append(single_frame_face)
                box_list.append(frame_data["boxes"])

            for frame_data in info_list:
                frame = cv2.cvtColor(frame_data["frames"], cv2.COLOR_BGR2RGB)
                frame_list.append(frame)

            return face_list, box_list, frame_list
        else:
            return [], []
